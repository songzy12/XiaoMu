...
class MatchPyramidAttention(BasicModel):
    def __init__(self, config):
        super(MatchPyramidAttention, self).__init__(config)
    ...
    def build(self):
        ...
        q_weighted = Embedding(self.config['vocab_size'], self.config['embed_size'], weights=[self.config['tfidf']], trainable = False)(query)
        show_layer_info('TFIDF', q_weighted)
        d_weighted = Embedding(self.config['vocab_size'], self.config['embed_size'], weights=[self.config['tfidf']], trainable = False)(doc)
        show_layer_info('TFIDF', d_weighted)

        q_embed = Multiply()([q_embed, q_weighted])
        show_layer_info('Multiply', q_embed)
        d_embed = Multiply()([d_embed, d_weighted])
        show_layer_info('Multiple', d_embed)
        ...
        print('dpool_size', self.config['dpool_size'][0], self.config['dpool_size'][1])
